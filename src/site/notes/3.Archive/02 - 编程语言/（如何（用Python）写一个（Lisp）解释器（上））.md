---
{"dg-publish":true,"dg-path":"02 - 编程语言/（如何（用Python）写一个（Lisp）解释器（上））.md","permalink":"/02 - 编程语言/（如何（用Python）写一个（Lisp）解释器（上））/","created":"2024-01-04T11:28:28.000+08:00","updated":"2025-02-25T10:50:14.700+08:00"}
---
...
这一章中，我们实现了一个简单的 Lisp 计算器，在[[3.Archive/02 - 编程语言/（如何（用Python）写一个（Lisp）解释器（下））\|下半部分]]中，我们将在此基础上写一个更完整的 Scheme 解释器。